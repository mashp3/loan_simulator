...
class PurchaseService {
  ...
  Future<void> _handlePurchase(PurchaseDetails purchaseDetails) async {
    ...
    if (purchaseDetails.status == PurchaseStatus.purchased ||
      ...
      print('⚠️ 購入エラー: ${purchaseDetails.error?.message}');
      print('⚠️ エラーコード: ${purchaseDetails.error?.code}');
  ...
  Future<bool> purchasePremium() async {
    ...
    try {
      ...
      if (response.notFoundIDs.isNotEmpty) {
        print('⚠️ プロダクトが見つかりません: ${response.notFoundIDs}');
        // 【修正】Google Play Console → ストアコンソール
        print('💡 ストアコンソールでプロダクトが正しく設定されているか確認してください');
        return false;
      }

      if (response.productDetails.isEmpty) {
        print('⚠️ プロダクト詳細が取得できません');
        return false;
      }
